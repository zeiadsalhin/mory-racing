        ...
        <v-dialog v-model="fullPlayer" :fullscreen="true" :hide-overlay="true" :opacity="0" close-delay="10000"
            ...
            <div v-if="authenticating" class="live flex flex-col m-20 w-[20rem] mx-auto text-center">
                ...
                <form @submit.prevent="handleSubmit">
                    <v-text-field :disabled="resultt ? true : false" variant="outlined" label="Tiktok Live ID*"
                        id="New Live ID" v-model="newLiveId" color="#ff0050" bg-color="grey-darken-4" spellcheck="false"
                        type="text" class="w-full mx-auto font-sans text-x" required />
                    ...
                    <v-btn @click="authenticating = false" type="button" v-if="resultt" variant="outlined"
                        :elevation="10" class="mt-[2rem] w-[14rem] mx-auto text-center">
                        Launch Game!</v-btn>
                </form>
